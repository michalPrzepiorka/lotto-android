...
public class LottoPlusActivity extends AppCompatActivity {
    ...
    private void getPosts() {
        ...
        JsonPlaceHolderApiLottoPlus jsonPlaceHolderApiLottoPlus = retrofit.create(JsonPlaceHolderApiLottoPlus.class);
        Call<List<LottoPlusPost>> call = jsonPlaceHolderApiLottoPlus.getPostsLottoPlus();

        call.enqueue(new Callback<List<LottoPlusPost>>() {
            @Override
            public void onResponse(@NotNull Call<List<LottoPlusPost>> call, @NotNull Response<List<LottoPlusPost>> response) {
                if (!response.isSuccessful()) {
                    textViewResult.setText("Code: " + response.code());
                    return;
                }

                List<LottoPlusPost> posts = response.body();
                if (posts != null) {
                    for (LottoPlusPost post : posts) {
                        String content = "";
                        content += "Date: " + post.getDate() + "\n";
                        content += "Numbers: " + post.getNumbers() + "\n\n";
            ...
            @Override
            public void onFailure(@NotNull Call<List<LottoPlusPost>> call, @NotNull Throwable t) {
    ...
    private void getData(String date, List<Integer> list) {
        ...
        JsonPlaceHolderApiLottoPlus jsonPlaceHolderApiLottoPlus = retrofit.create(JsonPlaceHolderApiLottoPlus.class);
        Call<LottoPlusDateHelper> call = jsonPlaceHolderApiLottoPlus.getDataLottoPlus(new LottoPlusDateHelper(date, list));

        call.enqueue(new Callback<LottoPlusDateHelper>() {
            @Override
            public void onResponse(@NotNull Call<LottoPlusDateHelper> call, @NotNull Response<LottoPlusDateHelper> response) {
                LottoPlusDateHelper body = response.body();
                if (body != null) {
            ...
            @Override
            public void onFailure(@NotNull Call<LottoPlusDateHelper> call, @NotNull Throwable t) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_lotto_plus);
        setUiViews();
        initializeListeners();
